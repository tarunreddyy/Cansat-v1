...
int updateRate = 1500;
long IndicatorRate = 1000;
int Buzzer = 32;

boolean Ind1State = false, Ind2State = false;
String TLE_Str1="",TLE_Str2="", TLE_Str3="";

void setup()
{
  Serial.begin(115200);
  Serial.println("Booting CanSat...");
  timer.setInterval(IndicatorRate, indicate1); //call indicate1 every second
  //timer.setInterval(, indicate2);
...
void TLE_Generate() //Generats a TLE string for transmission
{
  ...
  long runtime=0;
  runtime=millis()/100;
  runstr=String(runtime);
  //Generate TLE Strings
  st1 = st1 + latitude_st +" "+longitude_st +" "+Altitude_st+" "+Sat_No_st;
  Serial.println(st1);
  st2 = st2 + humidity_st +" "+Temp_st+" "+pressure_st+" "+runstr;
  Serial.println(st2);
  st3 = st3 + gyx_st +" "+ gyy_st+" "+ gyz_st+" "+acx_st+" "+acy_st+" "+acz_st+" "+magx_st+" "+magy_st+" "+magz_st;
  Serial.println(st3);
  TLE_Str1= st1;
  TLE_Str2= st2;
  TLE_Str3= st3;
...
void MPU()
{
  if ( imu.dataReady() )
  {
    ...
    if (debug)
    {
      Serial.println("Accel: " + String(acx) + ", " +String(acy) + ", " + String(acz) + " g");
      Serial.println("Gyro: " + String(gyx) + ", " +String(gyy) + ", " + String(gyz) + " dps");
  ...
  {
    Serial.println("IMU Dataset is being prepared");
  }
}
//---------------------------------------MPU-9250_END
...
void GPS()
{
  ...
  //delay(100);//delay removed for testing
